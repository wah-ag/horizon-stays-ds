...
`Staging Storybook` on the component's `Components` row in Airtable. If it is
empty, do not test — not local Storybook, not the story file. Wait, and say you
are waiting. Waiting is a correct outcome, not a failure to report.
...
Open the deployed staging build from `Staging Storybook`, in Claude in Chrome, and
open each story at `<Staging Storybook>/iframe.html?id=<story-id>&viewMode=story`.
Never start local Storybook, and never test from the story file.
...
token names live in `config/css/` — generated by `npm run build:tokens`, so read
it, never edit it.
